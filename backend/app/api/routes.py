...
from ..journal import compose_entries, upsert_note
...
# 롤업은 app/trades.py 한 곳에만 둔다 — 거래 내역과 매매일지가 공유한다.
from ..trades import trade_history_rows as _trade_history_rows
...
@router.get("/journal")
async def get_journal(request: Request, limit: int = 50) -> list[dict]:
    """종결 거래별 일지 — 진입 근거·래더 설계·손절/익절 근거·결과 (최신순).

    자동 서술은 저장하지 않고 매번 조립한다 (파생 뷰). 사용자 메모만 저장분."""
    return await asyncio.to_thread(
        compose_entries, request.app.state.db, int(limit)
    )


@router.put("/journal/{plan_id}/note")
async def put_journal_note(
    request: Request, plan_id: int, update: JournalNoteUpdate
) -> dict:
    """일지 메모 저장 (upsert). 종결 거래가 아니면 404."""
    try:
        return upsert_note(request.app.state.db, plan_id, update.note)
    except KeyError:
