        ...
        *,
        ...
        for attempt in range(attempts):
            ...
            if order.status == "filled":
                return order
            # GTX가 크로스로 즉시 거부(rejected)되거나 즉시 소멸(expired)하면
            # wait_seconds를 태우지 말고 곧장 다음 시도/폴백으로 (스펙 §5).
        ...
        # 폴백: reduce-only IOC 크로싱 리밋 (즉시 체결 아니면 소멸).
        quote = await self.get_quote(symbol)
